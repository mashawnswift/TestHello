...
public:
	...
	bool CBFuncAttChangeTrainingShowAirspeed( const int i_attIndex);
	bool CBFuncAttChangeTrainingCompassMode( const int i_attIndex);
	//}}DLC_ATTRIBUTECHANGE_FUNC_DECLERATION
...
private:
	...
	int m_ChangeIdAttTrainingShowAirspeed;
	int m_ChangeIdAttTrainingCompassMode;
	//}}DLC_ATTRIBUTECHANGE_VARIABLES_DECLERATION
	
	//{{DLC_HAGI_VARIABLES_DECLERATION
	HAGI  _abstract;//³éÏó½Úµã
	HAGI _txtTraining;//¾²Ì¬ÎÄ×Ö
	HAGI _txtAirspeed;//¿ÕËÙ×Ö·û
	HAGI _rcAirspeed;//¿ÕËÙ·½¿ò
	HAGI _txtBtn4;//°´¼ü4×Ö·û
	HAGI _txtBtn6;//°´¼ü6×Ö·ûFull/Arc
	HAGI _compassAbstract;//ÂÞÅÌ¸¸½Úµã
	HAGI _compassCircle;//ÂÞÅÌÔ²»·
	HAGI _txtNorth;//N
	HAGI _txtEast;//E
	HAGI _txtSouth;//S
	HAGI _txtWest;//W
	HAGI _compassMask;//ÂÞÅÌÕÚÕÖ

	//}}DLC_HAGI_VARIABLES_DECLERATION
	bool _bShowAirspeed;//ÏÔÊ¾×´Ì¬
	bool _bArcMode;//
